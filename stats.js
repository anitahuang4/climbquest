...
async function loadStats() {
  ...
  dailyHighEl.textContent = todayGames.length ? Math.max(...todayGames.map((g) => g.score)) : 0;
  weeklyHighEl.textContent = weekGames.length ? Math.max(...weekGames.map((g) => g.score)) : 0;

  recentGamesEl.innerHTML = games.slice(0, 8).map((game) => {
  ...
  new Chart(document.getElementById("scoreChart"), {
    type: "line",
    data: {
      labels: chartGames.map((g) => new Date(g.created_at).toLocaleDateString()),
      datasets: [
        {
          label: "Score",
